...
import { prioritizeAssets } from '@/lib/asset-utils'
...
export default function UniversalAssetViewer({ briefingDoc, datasets, caseFiles }: UniversalAssetViewerProps) {
  ...
  // Combine all assets and prioritize, then limit to 3
  const allAssets = useMemo(() => {
    const assets: Array<{
      type: 'overview' | 'file' | 'dataset'
      key: string
      fileName: string
      fileType: string
    ...
    processedCaseFiles.forEach(file => {
      assets.push({
        type: 'file',
        key: `file-${file.key}`,
    ...
    processedDatasets.forEach(dataset => {
      assets.push({
        ...
        fileName: dataset.metadata?.name || dataset.metadata?.title || dataset.fileName,
        fileType: dataset.fileType,
        content: dataset.content,
        metadata: dataset.metadata,
        originalValue: dataset.originalValue,
        isObject: dataset.isObject,
      })
    })
  ...
  const limitedAssets = prioritizedAssets

  // Render function for a single asset
  const renderAsset = (asset: typeof limitedAssets[0]) => {
    if (asset.type === 'overview') {
      return (
        <AssetRenderer
      ...
      return renderCaseFile({
        key: asset.key.replace('file-', ''),
        fileName: asset.fileName,
        fileType: asset.fileType,
        content: asset.content,
        originalFileType: asset.originalFileType || asset.fileType,
      })
    } else {
      // dataset
      const dataset = processedDatasets.find(d => d.key === asset.key.replace('data-', ''))
      if (dataset) {
  ...
  return (
    <div className="h-full flex flex-col bg-white border-r border-gray-200">
      ...
      ) : limitedAssets.length === 1 ? (
        // Single asset - show directly without tabs
        <div className="flex-1 overflow-y-auto p-6">
          {renderAsset(limitedAssets[0])}
        </div>
      ) : (
        // Multiple assets - show in tabs
        <Tabs defaultValue={limitedAssets[0].key} className="flex-1 flex flex-col">
          <TabsList className="mx-4 mt-4 !flex !flex-col !inline-flex h-auto w-auto overflow-y-auto">
            {limitedAssets.map((asset) => (
              <TabsTrigger key={asset.key} value={asset.key} className="gap-2 w-full justify-start">
                {asset.type === 'overview' ? (
                  <FileText className="h-4 w-4" />
                ) : asset.type === 'file' ? (
                  getFileIcon(asset.originalFileType || asset.fileType)
                ) : (
                  <Table className="h-4 w-4" />
                )}
                {asset.type === 'overview' ? 'Overview' : asset.fileName}
              </TabsTrigger>
            ))}
          </TabsList>

          {limitedAssets.map((asset) => (
            <TabsContent key={asset.key} value={asset.key} className="flex-1 overflow-y-auto p-6 mt-0">
              {renderAsset(asset)}
            </TabsContent>
          ))}
        </Tabs>
