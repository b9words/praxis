...
export default function SlidesRenderer({ content }: SlidesRendererProps) {
  const { slides, title, hasFrontmatter, hasSeparators } = useMemo(() => {
    // Parse Marp deck
    ...
    if (frontmatterMatch) {
      ...
      const titleMatch = frontmatterMatch[1].match(/title:\s*(.+)/i)
      if (titleMatch) {
    ...
    return { slides: parsedSlides, title: deckTitle, hasFrontmatter, hasSeparators }
  }, [content])
